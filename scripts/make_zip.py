...
def  main():
  ...
  installer = "\installer\IPlugFrameLib Installer.exe"
   
  if demo:
    installer = "\installer\IPlugFrameLib Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\IPlugFrameLib manual.pdf" 
  ]
  ...
  ZIPNAME = "IPlugFrameLib-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "IPlugFrameLib-v" + FULLVERSIONSTR + "-win-demo.zip"
